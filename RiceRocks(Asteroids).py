...
class Ship:
    ...
    def update(self):
        ...
        self.pos[0] = (self.pos[0] + self.radius + self.vel[0]) % (WIDTH + 2 * self.radius) - self.radius
        self.pos[1] = (self.pos[1] + self.radius + self.vel[1]) % (HEIGHT + 2 * self.radius) - self.radius       
        ...
        if self.thrust:
            self.vel[0] += self.forward[0] * 0.25
            self.vel[1] += self.forward[1] * 0.25
...
def group_collide(group, other_object):
    ...
    for sprite in group:
        if sprite.collide(other_object) == True:
            ...
            explosion = Sprite(other_object.pos, [0, 0], 0, 0,
...
def rock_spawner():
    ...
    rock_vel = [random.random() * score * 0.01 - score * 0.005, random.random() * score * 0.01 - score * 0.005]
    rock_avel = random.random() * .2 - .1
    if started:
        if len(rock_group) < 12 and dist(rock_pos, my_ship.get_position()) > asteroid_info.get_radius() + my_ship.get_radius() + 50:
